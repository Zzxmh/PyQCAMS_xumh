...
def extract_valid_data(output, r2_array, r3_array, theta_array):
    """
    提取有效的数据点并返回特征和标签。将 NaN 替换为 1000。
    """
    # 获取所有的索引（包括 NaN 数据）
    ...
    # Replace NaN with 1000 in labels
    labels = np.nan_to_num(labels, nan=1000)

    return features_flattened, labels


def main():
    data_folder_path = 'NN/dataset_XZC'
    fig_folder_path = 'NN/dataset_visualize_XZC'
